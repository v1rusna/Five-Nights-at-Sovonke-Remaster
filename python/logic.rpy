init 1 python in v1FNaSR:
    ...
    class FNaSR_Night6(FNaSR):
        def __init__(self):
            super(FNaSR_Night6, self).__init__()
            self.mainL.set_img(resources.images.bg[""])
            self.mainL.is_bulb = False
            self.mainL.is_tablet = False
            self.mainL.is_door = False

            self._is_start = False

        def start_mod(self): raise RuntimeError
        def quit_mod(self): raise RuntimeError
        def add_init_function(self, fn): pass
        def remove_init_function(self, fn): pass
        def __init_night_state(self): pass
